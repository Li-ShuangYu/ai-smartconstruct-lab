...
export const startTraining = (participationId: number, startNodeId: string) =>
  http.post<ApiResult<{ currentNodeId: string }>>(`/api/student/training-tasks/${participationId}/start`, { startNodeId }).then(r => r.data)

export const nextTraining = (participationId: number, nextNodeId: string, isEnd = false) =>
  http.post<ApiResult<{ currentNodeId: string; completed?: boolean }>>(`/api/student/training-tasks/${participationId}/next`, { nextNodeId, isEnd }).then(r => r.data)
