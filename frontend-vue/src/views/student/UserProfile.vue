      ...
      <div class="profile-layout">
        <aside class="profile-sidebar">
          <div class="avatar-card sidebar-block">
            ...
            <p class="user-role">{{ profile.className || '' }} | 学生</p>
            <div class="user-id">学号: {{ profile.studentNo || '-' }}</div>
          </div>

          <div class="info-card sidebar-block">
            <h3 class="mini-title">基本信息</h3>
            <div class="info-item"><span class="info-label">院系</span><span class="info-value">{{ profile.deptName || '-' }}</span></div>
            <div class="info-item"><span class="info-label">专业</span><span class="info-value">{{ profile.majorName || '-' }}</span></div>
...
import { getProfile, updatePassword } from '@/services/modules/student-dashboard.service'
import type { StudentProfile } from '@/services/types/dashboard.types'
...
.avatar-wrapper { width: 80px; height: 80px; border-radius: 50%; margin: 0 auto 16px; background: linear-gradient(135deg, #4F46E5, #818CF8); display: flex; align-items: center; justify-content: center; }
...
.info-value { font-weight: 500; color: #334155; max-width: 65%; text-align: right; }
