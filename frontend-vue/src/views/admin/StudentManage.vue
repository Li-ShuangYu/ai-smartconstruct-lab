<template>
  <div class="admin-page">
    ...
    <div class="data-grid">
      <div class="grid-header" style="grid-template-columns: 80px 1fr 1fr 1fr 1fr 150px;">
        <span>ID</span>
        <span>姓名</span>
        <span>学号</span>
        <span>部门ID</span>
      ...
      <div class="grid-body">
        <div v-for="row in data.records" :key="row.userId" class="grid-row" style="grid-template-columns: 80px 1fr 1fr 1fr 1fr 150px;">
          <span>{{ row.userId }}</span>
          <span>{{ row.realName }}</span>
          <span>{{ row.studentNo }}</span>
          <span>{{ row.deptId }}</span>
          <span>{{ row.majorId }} / {{ row.classId }}</span>
    ...
    <n-modal v-model:show="showModal" preset="card" :title="editingId ? '编辑学生' : '新增学生'" style="width: 500px;">
      <n-form :model="form" label-placement="top">
        ...
        <n-form-item label="专业ID">
          <n-input-number v-model:value="form.majorId" placeholder="请输入专业ID" style="width: 100%;" />
        </n-form-item>
        <n-form-item label="班级ID">
...
import { NButton, NModal, NForm, NFormItem, NInput, NInputNumber, useMessage } from 'naive-ui'
import * as api from '@/services/modules/admin.service'
import type { Student, PageResult } from '@/services/types/admin.types'
...
const form = reactive<Partial<Student>>({ realName: '', studentNo: '', deptId: 0, majorId: 0, classId: 0 })
...
function openModal(row: Student | null) {
  ...
  if (row) {
    Object.assign(form, { realName: row.realName, studentNo: row.studentNo, deptId: row.deptId, majorId: row.majorId, classId: row.classId })
  } else {
    Object.assign(form, { realName: '', studentNo: '', deptId: 0, majorId: 0, classId: 0 })
  }
...
async function save() {
  ...
  if (!form.studentNo?.trim()) { message.warning('请输入学号'); return }
  saving.value = true
  try {
    if (editingId.value) {
