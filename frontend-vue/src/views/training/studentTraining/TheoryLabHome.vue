  ...
  <div class="home-root">
    ...
    <div class="home-topbar">
      <div class="home-pill">
        ...
        <div class="ai-status-indicator" :class="aiStatus">
    ...
    <div class="home-hero" :class="{ centered: !classrooms.length }">
      ...
      <div class="home-card">
        ...
        <div class="home-card-bottom">
          ...
          <div class="home-actions">
            ...
            <button class="home-send" :disabled="!requirement.trim() || generating" @click="generate">
              <span>{{ generating ? '生成中…' : '进入课堂' }}</span>
      ...
      <div v-if="generating" class="home-generating">
        <div class="home-generating-spinner"></div>
        <span>AI 正在生成实训课件，请稍候...</span>
      </div>
...
// ─── Classrooms ───
interface Classroom { id: string; name: string; date: string; slides?: unknown[]; agents?: string[] }
...
function onPdfChange(e: Event) {
  ...
  if (!file) return
  if (file.size > 50 * 1024 * 1024) { error.value = 'PDF文件大小不能超过50MB'; return }
  pdfFile.value = file
  error.value = ''
}
...
function addModel(pid: string) {
  ...
  if (cfg) { if (!cfg.models) cfg.models = []; cfg.models.push(m.trim()) }
  newModel.value[pid] = ''; saveProviders()
}
function removeModel(pid: string, mi: number) {
  const cfg = (providerConfigs.value as Record<string, { models: string[] }>)[pid]
  if (cfg) cfg.models.splice(mi, 1); saveProviders()
}
...
function openRoom(id: string) {
  try {
    ...
    if (data?.slides?.length) {
      ...
      // 跳转到课堂演示页面
      router.push('/training/student-training/theory-lab')
    }
  } catch { /* ignore */ }
}
...
async function generate() {
  ...
  const session = {
    ...
    interactiveMode: interactiveMode.value,
  ...
  try {
    const res = await fetch('/ai/chat/stream', {
      ...
      body: JSON.stringify({
        messages: [{
          role: 'user',
          content: `你是一位教学专家。请根据以下实训需求生成一组教学课件内容，使用JSON格式返回。

实训需求：${requirement.value.trim()}

请严格按照以下 JSON 格式返回，不要包含其他内容：
{
  "title": "课件标题",
  "outlines": [
    { "title": "知识点1", "content": "详细讲解内容，包含概念解释和示例", "type": "theory" },
    { "title": "代码示例", "content": "\\\`\\\`\\\`python\\nprint('hello')\\\n\\\`\\\`\\\`", "type": "code" },
    { "title": "随堂测验", "content": "题目内容", "type": "quiz", "options": ["A", "B", "C", "D"], "answer": 0 }
  ]
}
...
- 包含至少一个代码示例和随堂测验`,
    ...
    const jsonMatch = fullText.match(/\{[\s\S]*"outlines"[\s\S]*\}/)
    ...
    if (courseData?.outlines?.length) {
      const id = 'cl_' + Date.now()
      localStorage.setItem(id, JSON.stringify({
        name: courseData.title || requirement.value.trim().slice(0, 30),
        slides: courseData.outlines,
        agents: [],
        date: new Date().toLocaleDateString(),
      }))
      const idx: string[] = JSON.parse(localStorage.getItem('cl_index') || '[]')
      idx.unshift(id)
      localStorage.setItem('cl_index', JSON.stringify(idx))
      loadRooms()
      sessionStorage.setItem('th_slides', JSON.stringify(courseData.outlines))
      sessionStorage.setItem('th_agents', JSON.stringify([]))
      sessionStorage.setItem('th_title', courseData.title || requirement.value.trim().slice(0, 30))
      error.value = ''
      // 跳转到课堂演示页
      router.push('/training/student-training/theory-lab')
    } else {
...
async function checkAI() {
  try {
    ...
    aiStatus.value = res.ok ? 'online' : 'offline'
  } catch {
...
<style scoped>
.home-back-btn {
  position: fixed; top: 16px; left: 16px; z-index: 51;
...
.home-root {
  ...
  padding: 16px 16px 0; position: relative; overflow-x: hidden;
}
...
.home-blob {
  ...
  opacity: .06; animation: pulse 4s ease-in-out infinite;
}
...
.home-pb:hover { background: #f1f5f9; color: #475569; }
...
.ai-status-dot { width: 5px; height: 5px; border-radius: 50%; }
.ai-status-indicator.checking .ai-status-dot { background: #f59e0b; }
.ai-status-indicator.online .ai-status-dot { background: #22c55e; }
.ai-status-indicator.offline .ai-status-dot { background: #ef4444; }
.ai-status-text { color: #94a3b8; font-weight: 500; }
.ai-status-indicator.online .ai-status-text { color: #16a34a; }
.ai-status-indicator.offline .ai-status-text { color: #dc2626; }
...
.home-greeting { display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 4px 10px 4px 6px; border-radius: 999px; border: 1px solid #f1f5f9; user-select: none; transition: .12s; }
.home-greeting:hover { background: #f8fafc; }
