    ...
    <div class="glass-card w-full h-full p-6 flex flex-col z-10">
      ...
      <div class="flex-1 flex flex-col lg:flex-row gap-6 min-h-0">
        ...
        <div class="flex-[1.8] flex flex-col bg-white/40 border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div class="bg-indigo-50/50 px-4 py-3 border-b border-indigo-100/50 flex items-center justify-between shrink-0">
          ...
          <div class="px-6 py-2.5 bg-white border-b border-gray-200 flex justify-between items-center shrink-0">
            <div class="relative w-64">
              <input
                v-model="searchQuery"
                type="text"
                placeholder="搜索学生姓名..."
                class="w-full pl-8 pr-3 py-1.5 bg-gray-50 border border-gray-200 rounded-md text-sm text-gray-700 focus:outline-none focus:bg-white focus:border-indigo-400 transition-colors shadow-sm"
              >
          ...
          <div class="grid grid-cols-12 gap-4 px-6 py-3 border-b border-gray-200/50 bg-gray-50/50 text-xs font-bold text-gray-500 uppercase tracking-wider shrink-0">
          ...
          <div class="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
            ...
            <div v-else class="h-full flex flex-col items-center justify-center text-gray-400 space-y-2 pb-6">
               <svg style="width: 32px; height: 32px; flex-shrink: 0;" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
               <span class="text-sm font-medium">无匹配学生</span>
            </div>
          </div>

          <div class="px-4 py-3 border-t border-gray-200/60 bg-white/40 flex items-center justify-between shrink-0">
            <button @click="prevPage" :disabled="currentPage === 1" class="px-3 py-1.5 rounded text-xs font-bold transition-colors" :class="currentPage === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 bg-indigo-50 hover:bg-indigo-100'">
              上一页
            </button>
            <div class="text-xs font-bold text-gray-500">
              {{ currentPage }} / {{ totalPages || 1 }}
            </div>
            <button @click="nextPage" :disabled="currentPage >= totalPages || totalPages === 0" class="px-3 py-1.5 rounded text-xs font-bold transition-colors" :class="currentPage >= totalPages || totalPages === 0 ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 bg-indigo-50 hover:bg-indigo-100'">
              下一页
            </button>
          </div>
...
const studentList = ref([
  ...
  { id: 7, name: '孙同学', isSubmitted: true, score: 95 },
  { id: 8, name: '周同学', isSubmitted: true, score: 88 },
  { id: 9, name: '吴同学', isSubmitted: false, score: 0 },
  { id: 10, name: '郑同学', isSubmitted: true, score: 72 },
])
...
const questionStats = ref([
  ...
  { id: 2, title: '轮函数 F 包含的非线性变换 τ 是由几个 S 盒构成的？', errorRate: 65 }, 
